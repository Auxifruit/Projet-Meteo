...
int existeFilsGauche(pAvl a) { // a pointeur sur l'arbre; vérifie si l'arbre a un fils gauche
    if(estVide(a)) { // verification arbre vide ou pas
...
z=insertionABR( z, 0 , 0 ,  data  , 0  ,0 , 0 , longi ,lat , 0  , 0  ,0 , 0, numstation);
}
if (r== 1)
parcours(z , m2);
else 
...
x=cosf (angle) ;
y=sinf (angle);
z=insertionABRvect( z, x, y,  data  , 0  ,0 , 0 , longi ,lat, 0 , 0 ,0 , 0, numstation);
}
...
z=insertionAVLNumstat( z, 0 , 0 ,  0  , max  , moy, min , 0 ,0 , 0 , 0  ,0 , 0 , numstation);
}
parcourstemp1 (z ,t2  );
fclose(t) ; 
fclose(t2) ; 
...
while (fscanf(t ,"%d,%d-%d-%dT%d:%d:%d+%d:%d,%f\n" , &annee , &jour , &mois , &heure ,&poubelle ,&poubelle , &poubelle ,&data )!=EOF)
{

z= insertionAVLdate (z, 0 , 0 ,  data  , 0  ,0 , 0 , 0 ,0 , heure  , jour  ,mois  , annee , 0);
}
parcourstemp2 ( z, t2 );
fclose(t) ; 
fclose(t2) ; 
...
{
z=insertionAVLdate( z, 0 , 0 ,  data  , 0  ,0 , 0 , 0 ,0 , heure  , jour  ,mois  , annee , numstation);
